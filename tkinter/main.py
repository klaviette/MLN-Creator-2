...
_current_tab = [1]       # mutable so closures see updates
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 2 — Inter-layer Network Creator with Set Operations
# ══════════════════════════════════════════════════════════════════════════════

# Scrollable canvas for Tab 2
canvas2 = tk.Canvas(tab2_frame, bg=BG, highlightthickness=0)
canvas2.pack(side="left", fill="both", expand=True)

body2    = tk.Frame(canvas2, bg=BG)
body2_id = canvas2.create_window((0, 0), window=body2, anchor="nw")

def _resize2(e): canvas2.itemconfig(body2_id, width=e.width)
def _scroll2(e): canvas2.configure(scrollregion=canvas2.bbox("all"))
canvas2.bind("<Configure>", _resize2)
body2.bind("<Configure>", _scroll2)

# Re-define mousewheel handler to be tab-aware, then rebind globally
def _mousewheel(e):
    widget = e.widget
    if isinstance(widget, (tk.Listbox, tk.Text)):
        return
...
# State: list of full paths of selected .net files
_net_files: list = []

# ── STEP 1 — SELECT .NET LAYER FILES ─────────────────────────────────────────
s1_outer, s1 = make_card(body2, "STEP 1 — SELECT .NET LAYER FILES")
s1_outer.pack(fill="x", **PAD)

tk.Label(s1, text="Add two or more .net files generated by the Intra-layer Creator.",
         font=F_SMALL, fg=SUBTEXT, bg=CARD).pack(anchor="w", pady=(0, 8))

net_list_frame, net_listbox = make_listbox(
    s1, selectmode=tk.EXTENDED, exportselection=False, height=7)
net_list_frame.pack(fill="x", pady=(0, 6))

net_status_lbl = tk.Label(s1, text="No files selected", font=F_SMALL, fg=SUBTEXT, bg=CARD)
net_status_lbl.pack(anchor="w", pady=(0, 8))

net_btn_row = tk.Frame(s1, bg=CARD)
net_btn_row.pack(anchor="w")

def _refresh_net_status():
    n = len(_net_files)
    net_status_lbl.config(
        text=f"{n} file(s) selected" if n else "No files selected",
        fg=SUCCESS if n else SUBTEXT,
    )
...
def _add_net_files():
    ...
    for fp in fps:
        if fp not in _net_files:
            _net_files.append(fp)
            net_listbox.insert(tk.END, os.path.basename(fp))
    _refresh_net_status()

def _remove_net_files():
    sel = net_listbox.curselection()
    if not sel:
        return
    for idx in reversed(sel):
        net_listbox.delete(idx)
        del _net_files[idx]
    _refresh_net_status()

def _clear_net_files():
    net_listbox.delete(0, tk.END)
    _net_files.clear()
    _refresh_net_status()

btn_add_net    = make_button(net_btn_row, "Add .NET Files\u2026", _add_net_files)
btn_remove_net = make_button(net_btn_row, "Remove Selected",    _remove_net_files)
btn_clear_net  = make_button(net_btn_row, "Clear All",          _clear_net_files)
btn_add_net.pack(side="left", padx=(0, 10))
btn_remove_net.pack(side="left", padx=(0, 10))
btn_clear_net.pack(side="left")

# ── STEP 2 — SET OPERATION ────────────────────────────────────────────────────
s2_outer, s2 = make_card(body2, "STEP 2 — SET OPERATION")
s2_outer.pack(fill="x", **PAD)

set_op_var = tk.StringVar(value="AND")

op_row = tk.Frame(s2, bg=CARD)
op_row.pack(fill="x")

for _op, _hint in [
    ("AND", "Edges present in ALL selected layers\n(intersection)"),
    ("OR",  "Edges present in ANY selected layer\n(union)"),
    ("NOT", "Edges in first layer but NOT in second\n(difference — requires exactly 2 files)"),
]:
    _rb_outer = tk.Frame(op_row, bg=CARD)
    _rb_outer.pack(side="left", padx=(0, 32), pady=(0, 4))
    tk.Radiobutton(
        _rb_outer, text=_op, variable=set_op_var, value=_op,
        font=(F_H2[0], F_H2[1]+1, "bold"), fg=TEXT, bg=CARD,
        activebackground=CARD, relief="flat", cursor="hand2",
    ).pack(anchor="w")
    tk.Label(
        _rb_outer, text=_hint, font=F_SMALL, fg=SUBTEXT, bg=CARD,
        justify="left", wraplength=220,
    ).pack(anchor="w")

# ── STEP 3 — OUTPUT FILE ──────────────────────────────────────────────────────
s3_outer, s3 = make_card(body2, "STEP 3 — OUTPUT FILE")
s3_outer.pack(fill="x", **PAD)

tk.Label(s3, text="Layer / file name:", font=F_H2, fg=TEXT, bg=CARD, anchor="w").pack(anchor="w")
out_name_var = tk.StringVar(value="inter_layer_result")
out_name_entry = tk.Entry(
    s3, textvariable=out_name_var, font=F_BODY, fg=TEXT, bg="#F8FAFC",
    relief="flat", bd=0,
    highlightthickness=1, highlightbackground=BORDER, highlightcolor=PRIMARY,
    width=48,
)
out_name_entry.pack(anchor="w", ipady=5, pady=(4, 14))

tk.Label(s3, text="Output directory:", font=F_H2, fg=TEXT, bg=CARD, anchor="w").pack(anchor="w")
...
make_button(out_dir_row2, "Browse\u2026", _browse_out_dir).pack(side="left", padx=(8, 0))

# ── STEP 4 — RESULT PREVIEW ───────────────────────────────────────────────────
s4_outer, s4 = make_card(body2, "STEP 4 — RESULT PREVIEW")
s4_outer.pack(fill="x", **PAD)

result_text2 = tk.Text(
    s4, font=F_MONO, bg="#1E1E2E", fg="#CDD6F4",
    insertbackground="white", relief="flat", bd=0,
    padx=14, pady=10, height=20, state=tk.DISABLED,
    highlightthickness=0,
)
result_text2.pack(fill="both", expand=True)

# ── Business logic for set operations ────────────────────────────────────────
def _parse_net_file(filepath):
    """Return (layer_name, node_list, edge_frozenset) from a custom .net file."""
...
def _run_set_operation():
    if len(_net_files) < 2:
        messagebox.showwarning("Not Enough Files",
            "Please add at least 2 .NET layer files.")
        return

    op = set_op_var.get()
    if op == "NOT" and len(_net_files) != 2:
        messagebox.showwarning("NOT Requires Exactly 2 Files",
            "The NOT operation computes:\n"
            "  (edges in file 1)  minus  (edges in file 2)\n\n"
            "Please select exactly 2 .NET files when using NOT.")
        return

    out_name = out_name_var.get().strip() or "inter_layer_result"
    out_dir  = out_dir_var.get().strip()
    if not out_dir:
        messagebox.showwarning("No Output Directory",
            "Please select an output directory in Step 3.")
        return

    try:
        parsed = [_parse_net_file(fp) for fp in _net_files]
    except Exception as exc:
        messagebox.showerror("Parse Error",
            f"Could not read a .NET file:\n{exc}")
        return

    edge_sets = [p[2] for p in parsed]

    if op == "AND":
        result_edges = edge_sets[0].copy()
        for es in edge_sets[1:]:
            result_edges &= es
    elif op == "OR":
        result_edges: set = set()
        for es in edge_sets:
            result_edges |= es
    else:  # NOT
        result_edges = edge_sets[0] - edge_sets[1]

    # Union of all node lists (order-preserved, deduplicated)
    seen_nodes: set = set()
    result_nodes: list = []
    ...
    out_path = os.path.join(out_dir, out_name + ".net")
    try:
        with open(out_path, 'w', encoding='utf-8') as fh:
            fh.write(out_name + '\n')
            fh.write(str(len(result_nodes)) + '\n')
            fh.write(str(len(result_edges)) + '\n')
        ...
        messagebox.showerror("Write Error",
            f"Could not save the output file:\n{exc}")
        return

    # Preview — cap at 500 lines to keep the UI responsive
    with open(out_path, encoding='utf-8') as fh:
        all_lines = fh.readlines()
    preview = ''.join(all_lines[:500])
    if len(all_lines) > 500:
        preview += f"\n\u2026 ({len(all_lines) - 500} more lines not shown)"

    result_text2.config(state=tk.NORMAL)
    result_text2.delete(1.0, tk.END)
    result_text2.insert(tk.END, preview)
    result_text2.config(state=tk.DISABLED)

    messagebox.showinfo("Success",
        f"Inter-layer network created ({op}).\n"
        f"  Nodes : {len(result_nodes)}\n"
        f"  Edges : {len(result_edges)}\n\n"
        f"Saved to:\n{out_path}")

# ── Action button (Tab 2) ─────────────────────────────────────────────────────
btn2_frame = tk.Frame(body2, bg=BG)
btn2_frame.pack(pady=(8, 28))
make_button(btn2_frame, "Run Set Operation  \u2192", _run_set_operation).pack()

def _bind_scroll2(widget):
    if not isinstance(widget, (tk.Listbox, tk.Text)):
        widget.bind("<MouseWheel>", _mousewheel)
    for child in widget.winfo_children():
        _bind_scroll2(child)
